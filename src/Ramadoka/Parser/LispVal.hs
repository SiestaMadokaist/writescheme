...
(
  ...
  evalIf :: Env -> LispVal -> LispVal -> LispVal -> ThrowsError LispVal
  ...
  apply fname = \_ -> throwError $ UnboundVar "no function defined as" fname
  ...
  evalString env expr = runIOThrows $ liftM show $ (liftThrows $ readExpr expr) `fmap` eval env
